...
function DoctorResultCard({ doctor, sponsored = false }: { doctor: DirectoryDoctor; sponsored?: boolean }) {
...
export function ResultList({ search }: { search: DerivedSearch }) {
  ...
  return (
    <section className="results-section" aria-label="Resultados da busca">
      ...
      <div className="organic-results">{visibleOrganic.map((doctor) => <DoctorResultCard doctor={doctor} key={doctor.id} />)}</div>
